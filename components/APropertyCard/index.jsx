...
import { adminEditListings } from '@/lib/store/slices/admin.reducer';
...
const SPropertyCard = ({listing}) => {
  ...
  const formatDate = (dateString) => {
    ...
    const isYesterday = new Date(now.getFullYear(), now.getMonth(), now.getDate() - 1).toDateString() === dateObj.toDateString();
  ...
  const verifyListing = (listing) => {
    const verifiedListing = {
      ...listing,
      stage: 'active',
      // verified: true,
    }
    dispatch(adminEditListings(verifiedListing));
  }

  return (
    <div className="b-property-card" >
      <div className="property-card__image" style={imageStyles} />
      <div className="property-card__content">
        <div className="property-card__price">
          <span>₦{formatPrice(listing.details.price)}</span> {listing.details.paymentType}
        </div>
