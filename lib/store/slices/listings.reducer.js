...
const filterListings = (listings, filter) => {
    ...
    const { location, listingType, propertyType, minMaxPrice, bedrooms, bathrooms, amenitiesSelected} = filter
    ...
    if(amenitiesSelected?.length > 0) {
        ...
        filteredListings = filteredListings.filter(listing => hasAllElements( listing.details.amenities, amenitiesSelected) && listing )
    }
...
export const selectFilteredListings = createSelector(
    [listingsSelector, selectFilters],
